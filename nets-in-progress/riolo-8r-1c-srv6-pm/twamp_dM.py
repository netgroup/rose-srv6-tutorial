...
MAXPACKETSENT = int(500)      # Number of packet exchanged between sender and reflector, modify to your choice
...
class TWAMPDelayMeasurement(Thread):
    ...
    def run(self):
        ...
        sniff(iface = self.interface, filter = "udp", prn=self.packetRecvCallback, stop_filter = self.stopfilter) # changed filter to UDP
...
class Reflector(TWAMPUtils):
        ...
        def sendReflectorDelayPacket(self,dstAddr,sequence_number,scale=0,multiplier=1,mBZ=0,SSender=0,ZSender=0,scaleSender=0,multiplierSender=1):
            ...
            ipv6_packet_inside.src = self.srcAddr # src ipv6 inner (reflector's addr)
            ...
            pkt = (ipv6_packet / srv6_header / ipv6_packet_inside / udp_packet / twamp_reflector)
...
class Sender(TWAMPUtils):
    ...
    def sendSenderDelayPacket(self,scale=0,multiplier=1):
        ...
        srv6_header = IPv6ExtHdrSegmentRouting()
        srv6_header.addresses = self.revSegList  # reversed order of segList in srv6 header
        srv6_header.segleft = len(self.revSegList) - 1 # -1 beacuse start from 0
        srv6_header.lastentry = len(self.revSegList) - 1

        ipv6_packet_inside = IPv6()
        ipv6_packet_inside.src = self.srcAddr # src ipv6 inner (sender's addr)
        ipv6_packet_inside.dst = self.dstAddr # dst ipv6 inner (reflector's addr)
        ...
        pkt = (ipv6_packet / srv6_header / ipv6_packet_inside / udp_packet / twampPaylod) # new pkt with srv6 header
